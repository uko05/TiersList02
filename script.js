...
const SITE_ID = 'starrailRankingPath';
let refreshSavedImageUI = () => {};

// ===== 「画像を1回生成する」ミッション(アカウント登録者限定・生涯1回・+20UP) =====
const MISSION_CLAIM_KEY = 'starrailRankingPathImage';
...
function saveImage() {
  ...
  html2canvas(grid, { useCORS: true, scale: 2 })
    ...
    .then(async (blob) => {
      ...
      const filename = `スタレ推しキャラランキング_運命_${yyyy}${mm}${dd}_${hh}${mi}${ss}.jpg`;
